package br.com.digitalhouse.open.api;
...
public interface ClienteControllerOpenAPI {
	...
	ResponseEntity<?> salvar(
			@ApiParam(name = "corpo", value = "Representação de um novo cliente", required = true) @Valid ClienteRequest clienteRequest);
...
//	List<ClienteResumoDTO> listarResumo();
	...
	ResponseEntity<Cliente> remover(Long id);
	...
	ResponseEntity<?> atualizar(
			@ApiParam(name = "corpo", value = "Representação de um novo cliente", required = true) @Valid ClienteRequest cliente,
			Long id);
}
